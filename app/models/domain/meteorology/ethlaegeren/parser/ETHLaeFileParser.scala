...
object ETHLaeFileParser {
  ...
  def parseAndSaveData(timestampToWriteData: DateTime, nrOfLines: Int, ethLaeFileDataLines: List[String], meteoService: MeteoService, fileName: String, statKonfig: StationKonfig, specialParamKonfig: Seq[SpecialParamKonfig]) = {
    ...
    val maximumDateDataWasRead: Option[DateTime] = meteoService.findMaxMeasurementDateForAStation(statNr).headOption.map(maxDate => StringToDate.formatOzoneDate.parseDateTime(maxDate.stripPrefix("to_date('").replaceAll("'", "").stripSuffix(", DD.MM.YYYY HH24:MI:SS)")))
