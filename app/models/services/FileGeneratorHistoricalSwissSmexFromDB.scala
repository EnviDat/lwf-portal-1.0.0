...
class FileGeneratorHistoricalSwissSmexFromDB(meteoService: MeteoService) extends FileGenerator {
  ...
  def generateFiles(): List[FileInfo] = {
    ...
    allOrganisations.filter(_.organisationNr == 2).flatMap(o => {
      ...
      val allFilesDataGenerated = allStations.filter(st => stationNumbersConfigured.contains(st.stationNumber)).flatMap(station => {
        ...
        val allEinfDatesForStation = meteoService.getAllEinfdates(station.stationNumber, StringToDate.stringToDateConvert("01-01-2018 00:00:00"), StringToDate.stringToDateConvert("19-11-2018 00:00:00"),167, 193,"md2018","mdat2018" )
        allEinfDatesForStation.map(einfDatForData => {
        ...
        val latestMeteoDataForStation: Seq[MeteoDataRow] = meteoService.getLastMeteoDataForStationForDate(station.stationNumber, einfDatForData).sortBy(_.dateReceived)
      ...
      Logger.debug(s"Data lines to be written for CR10x stations: ${dataLinesToBeWrittenCR10.mkString("\n")}")
        ...
        val einfDate =   allEinfDates.max.toDateTime()
