...
class WeeklyPrecipitationService(config: ConfigurationPreciVordemwaldData, meteoService: MeteoService) {
  def getAndSendDataForLastWeekVORB() = {
   val ottPulvioMeasurement: Seq[WeeklyPreciVordemwaldDataRow] = meteoService.getLastOneWeekDataForStation(config.stationNrPreciVordemwaldB, config.messartPreciVordemwaldB)
  ...
  def getAndSendDataForLastWeekVORF() = {
    val ottPulvioMeasurement: Seq[WeeklyPreciVordemwaldDataRow] = meteoService.getLastOneWeekDataForStation(config.stationNrPreciVordemwaldF, config.messartPreciVordemwaldF)
    val emailList = config.emailUserListPreciVordemwald.split(";").toSeq
