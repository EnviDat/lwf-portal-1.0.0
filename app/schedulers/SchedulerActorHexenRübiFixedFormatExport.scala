...
class SchedulerActorHexenRübiFixedFormatExport @Inject()(configuration: Configuration, meteoService: MeteoService)(implicit ec: ExecutionContext) extends Actor {
  ...
  /*def writeFile(configMeteoSwiss: ConfigurationMeteoSchweizData, configHexenrubi: ConfigurationCR10XData): Unit ={
    ...
    val pathForTempFiles = configMeteoSwiss.pathForTempFiles //change it as it is temporarily set
    ...
    Logger.info(s"Destination for archive files ${pathForArchivedFiles + "generatedArchivedFiles" + CurrentSysDateInSimpleFormat.dateNow + "_HEX_.zip"} ")
